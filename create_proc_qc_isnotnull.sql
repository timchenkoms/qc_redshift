...
create or replace procedure proc_qc_not_null(
...
DECLARE
    ...
    lv_test_type varchar(100) := 'not null';
    rec RECORD;
...
BEGIN
    ...
    lv_sql = lv_sql||' where ('||replace(p_column_names,',',' is null or ')||' is null)';
    ...
    if lv_result_error_count- nvl(p_skip_errors,0) <= 0 then
        ...
        lv_result_message = lv_result_message||' have NULL value.';
        ...
        lv_sql = 'select '||lv_column_names||' as error_data from '||p_schema_name||'.'||p_table_name;
        lv_sql = lv_sql||' where ('||replace(p_column_names,',',' is null or ')||' is null)';
		lv_sql = lv_sql||decode(p_subset_condition, null,'','','',' and '||p_subset_condition);
