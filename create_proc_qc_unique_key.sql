...
create or replace procedure proc_qc_uniqueness(
...
DECLARE
    ...
    lv_test_type varchar(100) := 'uniqueness';
    rec RECORD;
...
BEGIN
    ...
    lv_sql = lv_sql||'select distinct '||p_column_names||', count(1) from '||p_schema_name||'.'||p_table_name;
	lv_sql = lv_sql||decode(p_subset_condition, null,'','','',' where '||p_subset_condition);
    lv_sql = lv_sql||' group by '||p_column_names||' having count(1) > 1)';
    ...
    if lv_result_error_count - nvl(p_skip_errors,0) <= 0 then
        ...
        lv_result_message = lv_result_message||' are not unique.';
        ...
        lv_sql = lv_sql||'select distinct '||p_column_names||' from '||p_schema_name||'.'||p_table_name;
		lv_sql = lv_sql||decode(p_subset_condition, null,'','','',' where '||p_subset_condition);
        lv_sql = lv_sql||' group by '||p_column_names||' having count(1) > 1)';
