...
/*
create or replace procedure proc_qc_foreign_key(
    ...
    p_domain_schema_name varchar(100),
    p_domain_table_name varchar(100),
    p_domain_column_names varchar(100))
    language plpgsql
as
$$
DECLARE
    ...
    lv_test_type varchar(100) := 'foreign key';
    rec RECORD;
    lv_column_names varchar(1000) = '';
    lv_column_name varchar(200);
BEGIN
    SELECT CONVERT_TIMEZONE('US/Eastern', cast(TIMEOFDAY() as timestamp)) INTO lv_starttime;
    lv_input_parameters = 'p_domain_schema_name = '||p_domain_schema_name;
    lv_input_parameters = lv_input_parameters||', p_domain_table_name = '||p_domain_table_name;
    lv_input_parameters = lv_input_parameters||', p_domain_column_names = '||p_domain_column_names;
    ...
    if p_column_names is NULL then
		...
		p_order_run, p_schema_name, p_table_name,p_column_names,p_skip_errors,
          lv_input_parameters, -1, lv_result_message,NULL, NULL, NULL);
        return;
    end if;

    if p_domain_column_names is NULL then
        lv_result_message = 'The list of domain column names in the table '||p_domain_schema_name||'.'||p_domain_table_name||' should include at least one column name.';
        CALL {{currentschema}}.proc_insert_test_result(lv_test_type,p_test_group,p_test_action,p_test_time, lv_starttime,
		p_order_run, p_schema_name, p_table_name,p_column_names,p_skip_errors,
          lv_input_parameters, -1, lv_result_message,NULL, NULL, NULL);
        return;
    ...
    CALL {{currentschema}}.proc_check_table(p_domain_schema_name, p_domain_table_name, p_domain_column_names, lv_result_code, lv_result_message);
    if lv_result_code <> 1 then
        CALL {{currentschema}}.proc_insert_test_result(lv_test_type,p_test_group,p_test_action,p_test_time, lv_starttime,
		p_order_run, p_schema_name, p_table_name,p_column_names,p_skip_errors,p_subset_condition,
          lv_input_parameters, -1, lv_result_message,NULL, NULL, NULL);
	...
	CALL {{currentschema}}.proc_match_column_type(p_schema_name, p_table_name, p_column_names, p_domain_schema_name, p_domain_table_name, p_domain_column_names, lv_result_code, lv_result_message);
    if lv_result_code <> 1 then
        CALL {{currentschema}}.proc_insert_test_result(lv_test_type,p_test_group,p_test_action,p_test_time, lv_starttime,
		p_order_run, p_schema_name, p_table_name,p_column_names,p_skip_errors,p_subset_condition,
          lv_input_parameters, -1, lv_result_message,NULL, NULL, NULL);
	...
	lv_sql = lv_sql||' minus ';
    lv_sql = lv_sql||'select '||p_domain_column_names||' from '||p_domain_schema_name||'.'||p_domain_table_name||')';

    EXECUTE lv_sql INTO lv_result_error_count;
    if lv_result_error_count - nvl(p_skip_errors,0) <= 0 then
        CALL {{currentschema}}.proc_insert_test_result(lv_test_type,p_test_group,p_test_action,p_test_time, lv_starttime,
		p_order_run, p_schema_name, p_table_name,p_column_names,p_skip_errors,p_subset_condition,
        lv_input_parameters, 1, NULL,NULL, lv_result_error_count, NULL);
    else
        ...
        lv_result_message = lv_result_message||' are not present in the table '||p_domain_schema_name||'.'||p_domain_table_name||', column(s) '||p_domain_column_names||'.';
		
		/*
		lv_result_error_data = p_column_names||chr(10);
		FOR rec IN EXECUTE lv_sql
        LOOP
            EXIT WHEN length(lv_result_error_data||rec.error_data||chr(10)) > 4000;
            lv_result_error_data = lv_result_error_data||rec.error_data||chr(10);
        END LOOP;
		*/
		
        CALL {{currentschema}}.proc_insert_test_result(lv_test_type,p_test_group,p_test_action,p_test_time, lv_starttime,
		p_order_run, p_schema_name, p_table_name,p_column_names,p_skip_errors,p_subset_condition,
        lv_input_parameters, 0, lv_result_message,NULL, lv_result_error_count,lv_result_error_data);
    end if;
