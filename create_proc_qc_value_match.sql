...
create or replace procedure proc_qc_value_match(
...
DECLARE
    ...
    lv_test_type varchar(100) := 'value match';
    rec RECORD;
    lv_column_names varchar(4000) = '';
    lv_column_name varchar(200);
	lv_value_set varchar(1000);
...
BEGIN
    ...
    lv_input_parameters = 'p_value_set = '||nvl(p_value_set,'NULL');
	...
	if lv_mode not in ('in','not in') then
	...
	if mod(regexp_count(p_value_set,''''),2) = 1 then
        lv_result_message = 'The parameter p_value_set has odd number of apostrophes to test data in the table '||p_schema_name||'.'||p_table_name||', column(s)' ||p_column_names||'.';
        CALL {{currentschema}}.proc_insert_test_result(lv_test_type,p_test_group,p_test_action,
		p_test_description, p_test_time, lv_starttime,
		p_order_run, p_schema_name, p_table_name,p_column_names,p_skip_errors,p_subset_condition,
          lv_input_parameters, -1, lv_result_message,NULL, NULL, NULL, NULL);
	...
	if regexp_count(p_value_set,';') > 0 or regexp_count(p_column_names,',') > 0 then
		lv_value_set = 'select '||replace(p_value_set, ';', ' union select ');
	else
		lv_value_set = 'select '||replace(p_value_set, ',', ' union select ');
	end if;
    ...
    if lv_mode = 'in' then
        ...
        lv_sql = lv_sql||' minus ('||lv_value_set||'))';
    elsif lv_mode = 'not in' then
        lv_sql = 'select count(1) from ';
        lv_sql = lv_sql||'(select '||p_column_names||' from '||p_schema_name||'.'||p_table_name;
		lv_sql = lv_sql||decode(p_subset_condition, null,'','','',' where '||p_subset_condition);
        lv_sql = lv_sql||' intersect ('||lv_value_set||'))';
    else
        lv_sql = '';
    end if;
    ...
    if lv_result_error_count - nvl(p_skip_errors,0) <= 0 then
        ...
        lv_result_message = lv_result_message||' do not pass a test "'||p_mode||'" with the match-value_set ('||p_value_set||').';
